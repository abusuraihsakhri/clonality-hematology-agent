"""
Clinical Algorithmic Engine & Guideline Rules for ClonoMind: IGH & TCR Gene Rearrangement Clonality Distributed Component Classifier.
...
class ClinicalDomainEngine:
    GUIDELINE = "WHO 5th Edition / ICC Hematolymphoid 2022"
    ...
    def evaluate_primary_index(cls, value: float) -> Optional[Dict[str, Any]]:
        if value > cls.PRIMARY_BASELINE_LIMIT:
            return {
                ...
                "finding": f"Observed value ({value:.2f}) exceeds WHO 5th Edition / ICC Hematolymphoid 2022 clinical baseline limit ({cls.PRIMARY_BASELINE_LIMIT:.1f}).",
    ...
    def evaluate_biomarker_concordance(cls, status_flag: str, biomarkers: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "DISCORDANT" in status_upper or "EQUIVOCAL" in status_upper or "MUTANT" in status_upper:
            return {
                ...
                "recommendation": f"Order reflex confirmatory testing per WHO 5th Edition / ICC Hematolymphoid 2022 clinical recommendations.",
            }
        return None
