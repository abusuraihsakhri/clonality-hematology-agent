...
**Clonality Hematology Agent** is an advanced analytical and computational platform implementing BIOMED-2 IGH/TCR Gene Rearrangement Clonality Classifier.
...
- **`PeakRatioCalculatorAgent`**: Specialized Sub-Agent 1 for clonality-hematology-agent
- **`PolyClonalityFilterAgent`**: Specialized Sub-Agent 2 for clonality-hematology-agent
- **`BMRDMarkerFinderAgent`**: Specialized Sub-Agent 3 for clonality-hematology-agent
...
```bash
docker build -t clonality-hematology-agent .
docker run -p 8000:8000 clonality-hematology-agent
```
